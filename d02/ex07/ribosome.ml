...
let () =
  Random.self_init ();
  let r = generate_rna (generate_helix 999) in
  Printf.printf "%s -> " (rna_to_string r);
  print_list (fun (a,b,c) ->
      print_string (nucleobase_to_string a ^
                    nucleobase_to_string b ^
                    nucleobase_to_string c))
    (generate_bases_triplets r);
  print_char '\n';
  print_endline "*********************";
  print_string (string_of_protein (decode_rna r));
  print_endline "*********************";
  print_string (string_of_protein (decode_rna [A;U;G; U;G;G]))
